...
class MainHub():
    ...
    def resegment_track(self, past_key_frames: deque[int] =deque(),
        ...
        Args:
            ...
            generated by TRACK or MULTI_TRACK. Defaults to []. assumed in ascending order
            new_frames (list[int], optional): List of new resegmented 
            frames from CVAT specified by User Input. Defaults to []. assumed in ascending order
        ...
        if not multithreading:
            ...
            try:
                for i in tqdm(range(len(new_frames)), "Processing new frame to past key frames in single iteration: "):
                    ...
                    #case where past key frames exists
                    elif len(past_key_frames) > 0:
                        end_frame = past_key_frames.popleft() - 1
...
def arg_main(sam_args=sam_args, segtracker_args=segtracker_args, aot_args=aot_args, job_id: int = -1, 
    ...
    if resegment:
        ...
        key_frame_arr.extend(list(new_frames))
